(function tesseraInstrumentAdapter(global) {
	...
	const glyphs = ["column", "row", "connect", "square", "checker", "diamond", "dot", "ring", "solid", "empty"];
	...
	const presets = {
		...
		// The yellow maze frame: the darkest tone runs down in columns, the next runs
		// across in rows, the next is a joined bar that picks its own axis, all in
		// one black — the light drops out to the yellow ground.
		maze: { invert: false, levels: "equalize", background: "#f7d358", colorMode: "palette", columns: 56, contrast: 1.2, gap: 0.14, glyph1: "column", glyph2: "row", glyph3: "connect", glyph4: "empty", merge: true, palette: ["#000000"], roundness: 0, scale: 0.62, tiers: 4, weighting: "flat" },
		// Blue bars on black: thin dark blue in the shadows, wide light blue in the light.
		signal: { invert: false, levels: "auto", background: "#000000", colorMode: "palette", columns: 44, contrast: 1.8, curve: 1, gap: 0.16, glyph1: "empty", glyph2: "column", glyph3: "column", merge: true, minWeight: 0.2, palette: ["#0000ff", "#6a9cff"], roundness: 0, scale: 0.8, tiers: 3, weighting: "ramp" },
		// The figure: checker for the body, white rungs for the light, orange staves.
		figure: { invert: false, levels: "auto", background: "#1b1d20", colorMode: "palette", columns: 72, contrast: 1.5, gap: 0.12, glyph1: "empty", glyph2: "checker", glyph3: "row", glyph4: "column", merge: true, palette: ["#c9b99a", "#f2f2f2", "#ea5a36"], roundness: 0, scale: 0.62, tiers: 4, weighting: "flat" },
		// The rooster: blue checker in the dark, light-blue diamonds, pale grey staves.
		rooster: { invert: false, levels: "auto", background: "#ffffff", colorMode: "palette", columns: 64, contrast: 1.4, gap: 0.05, glyph1: "checker", glyph2: "diamond", glyph3: "column", glyph4: "empty", merge: true, palette: ["#0000ff", "#5b8ff9", "#e4e5e7"], roundness: 0, scale: 0.9, tiers: 4, weighting: "flat" },
		// The Martens icon set, after John Provencher's own frames: rings, squares, dots.
		icons: { invert: false, levels: "auto", background: "#000000", colorMode: "palette", columns: 56, contrast: 1.3, glyph1: "empty", glyph2: "ring", glyph3: "square", glyph4: "dot", glyph5: "ring", glyph6: "square", merge: false, palette: ["#22a822", "#ff1fae", "#ff2a1a", "#e6c020", "#2436ff"], scale: 0.86, tiers: 6, weighting: "flat" },
		// The default: ink on paper. The subject (usually the darkest thing) is
		// drawn in heavy bars, the light drops out — the way the rooster and the
		// figure frames read. Invert puts the heaviest tier on the darkest tones.
		ink: { background: "#ffffff", colorMode: "palette", columns: 64, contrast: 1.4, curve: 1, gap: 0.08, glyph1: "empty", glyph2: "column", glyph3: "column", glyph4: "column", invert: true, levels: "equalize", merge: true, minWeight: 0.18, palette: ["#0000ff"], roundness: 0, scale: 0.92, tiers: 4, weighting: "ramp" },
		// Ours: red and paper on black.
		supermega: { invert: false, levels: "auto", background: "#050505", colorMode: "palette", columns: 90, contrast: 2.2, curve: 1, gap: 0.08, glyph1: "empty", glyph2: "dot", glyph3: "column", glyph4: "connect", merge: true, minWeight: 0.3, palette: ["#f61515", "#f61515", "#f3efe7"], roundness: 1, scale: 0.8, tiers: 4, weighting: "ramp" },
	};

	// Opens on the bold look (Signal: chunky bars, the Base film's blue-on-black
	// frame) and the picture's own shape; Base's exact settings are one preset away.
	const defaults = {
		...presets.base,
		...presets.signal,
		...presets.ink,
	...
	const schema = {
		...
		fields: {
			...
			underlay: { type: "number", label: "Underlay", section: "colour", minimum: 0, maximum: 0.9, step: 0.01, format: pct, unit: "%", hint: "The picture itself, faintly, under the marks." },
			...
			fit: { type: "enum", label: "Fit", section: "place", choices: ["cover", "contain"], visibleWhen: { notEquals: "source", target: "aspect" } },
	...
	const definition = Object.freeze({
		...
		capabilities: [
			"configure", "undo", "redo", "copy-config", "reset-configuration", "randomize",
	...
	global.SUPERMEGA_INSTRUMENT_ADAPTERS.tessera = Object.freeze({
		create(options) {
			if (!options || typeof options.nextVariant !== "function") {
				throw new TypeError("The Tessera adapter requires a nextVariant callback.");
			}
