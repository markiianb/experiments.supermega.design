/* SUPERMEGA Instrument 0.1.0-4b3001d2c6ef */
var SUPERMEGA_INSTRUMENT = (function() {
  ...
  function enhanceInstrument(root, initialState = createInitialInstrumentState()) {
    ...
    const settle = (n) => Number(n.toFixed(10));
    const paintLightPad = (surface, event) => {
      var _a2, _b2, _c2, _d2, _e2;
      const rect = surface.getBoundingClientRect();
      const rawX = (event.clientX - rect.left) / (rect.width || 1);
      const rawY = (event.clientY - rect.top) / (rect.height || 1);
      const min = Number((_a2 = surface.dataset.instrumentElevationMin) != null ? _a2 : "0");
      const max = Number((_b2 = surface.dataset.instrumentElevationMax) != null ? _b2 : "90");
      const span = max - min;
      const dx = (rawX - 0.5) / 0.5;
      const dy = -((rawY - 0.5) / 0.5);
      const reach = Math.hypot(dx, dy);
      const radius = Math.min(1, reach);
      const previous = Number((_c2 = surface.dataset.instrumentAzimuth) != null ? _c2 : "0");
      const snap = (reading, step) => Number.isFinite(step) && step > 0 ? Math.round(reading / step) * step : reading;
      const azimuthStep = Number((_d2 = surface.dataset.instrumentAzimuthStep) != null ? _d2 : "1");
      const elevationStep = Number(
        (_e2 = surface.dataset.instrumentElevationStep) != null ? _e2 : "1"
      );
      const azimuth = reach === 0 ? previous : settle(
        (snap(Math.atan2(dy, dx) * 180 / Math.PI, azimuthStep) % 360 + 360) % 360
      );
      const elevation = span > 0 ? settle(
        Math.min(
          max,
          Math.max(min, snap(max - radius * span, elevationStep))
        )
      ) : max;
      const bearing = azimuth * Math.PI / 180;
      surface.dataset.instrumentAzimuth = String(azimuth);
      surface.style.setProperty(
        "--instrument-light-x",
        String(settle(0.5 + 0.5 * radius * Math.cos(bearing)))
      );
      surface.style.setProperty(
        "--instrument-light-y",
        String(settle(0.5 - 0.5 * radius * Math.sin(bearing)))
      );
      surface.dispatchEvent(
        new CustomEvent("instrument:light", {
          bubbles: true,
          detail: { azimuth, elevation }
        })
      );
    };
    const paintAngleDial = (dial, event) => {
      const control = dial.closest(".instrument-angle");
      const input = control == null ? void 0 : control.querySelector(
        ".instrument-angle-input"
      );
      ...
      if (dx === 0 && dy === 0) return;
      const east = Math.atan2(-dy, dx) * 180 / Math.PI;
      const origin = control.dataset.instrumentOrigin === "right" ? "right" : "top";
      const clockwise = control.dataset.instrumentClockwise !== "false";
      const raw = origin === "top" ? 90 - east : east;
      const directed = clockwise ? raw : -raw;
      const min = Number(input.min === "" ? 0 : input.min);
      const max = Number(input.max === "" ? 360 : input.max);
      const step = Number(input.dataset.instrumentStep || input.step) || 1;
      const stepped = Math.round(directed / step) * step;
      const span = max - min;
      const next = input.dataset.instrumentWrap === "true" && span > 0 ? settle(min + ((stepped - min) % span + span) % span) : settle(Math.min(max, Math.max(min, stepped)));
      if (Number(input.value) === next) return;
      input.value = String(next);
      control.style.setProperty(
        "--instrument-angle",
        `${angleRotation(next, origin, clockwise)}deg`
      );
      input.dispatchEvent(new Event("input", { bubbles: true }));
    };
    const paintPad = (surface, event) => {
      if (surface.classList.contains("instrument-angle-dial")) {
        paintAngleDial(surface, event);
        return;
      }
      if (surface.classList.contains("instrument-light-pad-surface")) {
        paintLightPad(surface, event);
        return;
      }
      const rect = surface.getBoundingClientRect();
      ...
      const pad = surface.closest(".instrument-vector-pad");
      const axes = pad == null ? void 0 : pad.querySelectorAll(
        ".instrument-vector-pad-axis-value"
      );
      const project = (axis, unit) => {
        var _a2, _b2;
        if (!axis) return unit;
        const min = Number((_a2 = axis.getAttribute("aria-valuemin")) != null ? _a2 : "0");
        const max = Number((_b2 = axis.getAttribute("aria-valuemax")) != null ? _b2 : "1");
        const step = Number(axis.dataset.instrumentStep || "0") || 0;
        const raw = min + unit * (max - min);
        const stepped = step > 0 ? Math.round(raw / step) * step : raw;
        return settle(Math.min(max, Math.max(min, stepped)));
      };
      const detail = { x: project(axes == null ? void 0 : axes[0], x), y: project(axes == null ? void 0 : axes[1], y) };
      if (axes == null ? void 0 : axes.length) {
        axes.forEach((axis, index) => {
          const reading = index === 0 ? detail.x : detail.y;
          const step = Number(axis.dataset.instrumentStep || "0.01");
          const decimals = String(step).includes(".") ? String(step).split(".")[1].length : 0;
          axis.setAttribute("aria-valuenow", String(reading));
          const unitLabel = axis.querySelector(
            ".instrument-vector-pad-axis-unit"
          );
          axis.textContent = reading.toFixed(decimals);
          if (unitLabel) axis.appendChild(unitLabel);
        });
      }
      surface.dispatchEvent(
        new CustomEvent("instrument:vector", { bubbles: true, detail })
      );
    };
    const lightAxisFocusStart = /* @__PURE__ */ new WeakMap();
    const AXIS_VALUE_SELECTOR = ".instrument-light-pad-axis-value, .instrument-vector-pad-axis-value";
    const readLightAxis = (axis) => {
      var _a2, _b2, _c2, _d2, _e2;
      const surface = (_a2 = axis.closest(".instrument-light-pad")) == null ? void 0 : _a2.querySelector(".instrument-light-pad-surface");
      const wrap = axis.dataset.instrumentWrap === "true";
      return {
        max: Number((_b2 = axis.getAttribute("aria-valuemax")) != null ? _b2 : "0"),
        min: Number((_c2 = axis.getAttribute("aria-valuemin")) != null ? _c2 : "0"),
        step: Number((_d2 = axis.dataset.instrumentStep) != null ? _d2 : "1"),
        surface,
        value: Number((_e2 = axis.getAttribute("aria-valuenow")) != null ? _e2 : "0"),
        wrap
      };
    };
    const writeLightAxis = (axis, next) => {
      var _a2, _b2, _c2, _d2, _e2, _f, _g, _h, _i, _j;
      const { step, surface, wrap } = readLightAxis(axis);
      const decimals = String(step).includes(".") ? String(step).split(".")[1].length : 0;
      const vectorPad = axis.closest(".instrument-vector-pad");
      axis.setAttribute("aria-valuenow", String(next));
      if (!vectorPad) {
        axis.setAttribute("aria-valuetext", `${next.toFixed(decimals)} degrees`);
      }
      const unit = axis.querySelector(
        ".instrument-light-pad-axis-unit, .instrument-vector-pad-axis-unit"
      );
      axis.textContent = next.toFixed(decimals);
      if (unit) axis.appendChild(unit);
      if (vectorPad) {
        const values2 = vectorPad.querySelectorAll(
          ".instrument-vector-pad-axis-value"
        );
        const indicator = vectorPad.querySelector(
          ".instrument-vector-pad-indicator"
        );
        const unitPosition = (node) => {
          var _a3, _b3, _c3;
          if (!node) return 0;
          const low = Number((_a3 = node.getAttribute("aria-valuemin")) != null ? _a3 : "0");
          const high = Number((_b3 = node.getAttribute("aria-valuemax")) != null ? _b3 : "1");
          const now = Number((_c3 = node.getAttribute("aria-valuenow")) != null ? _c3 : "0");
          return high === low ? 0 : clampUnit((now - low) / (high - low));
        };
        indicator == null ? void 0 : indicator.style.setProperty(
          "--instrument-pad-x",
          String(settle(unitPosition(values2[0])))
        );
        indicator == null ? void 0 : indicator.style.setProperty(
          "--instrument-pad-y",
          String(settle(unitPosition(values2[1])))
        );
        vectorPad.dispatchEvent(
          new CustomEvent("instrument:vector", {
            bubbles: true,
            detail: {
              x: Number((_b2 = (_a2 = values2[0]) == null ? void 0 : _a2.getAttribute("aria-valuenow")) != null ? _b2 : "0"),
              y: Number((_d2 = (_c2 = values2[1]) == null ? void 0 : _c2.getAttribute("aria-valuenow")) != null ? _d2 : "0")
            }
          })
        );
        return;
      ...
      if (!surface) return;
      if (wrap) surface.dataset.instrumentAzimuth = String(next);
      const pad = axis.closest(".instrument-light-pad");
      const values = pad == null ? void 0 : pad.querySelectorAll(
        ".instrument-light-pad-axis-value"
      );
      const azimuth = Number((_f = (_e2 = values == null ? void 0 : values[0]) == null ? void 0 : _e2.getAttribute("aria-valuenow")) != null ? _f : "0");
      const elevation = Number((_h = (_g = values == null ? void 0 : values[1]) == null ? void 0 : _g.getAttribute("aria-valuenow")) != null ? _h : "0");
      const min = Number((_i = surface.dataset.instrumentElevationMin) != null ? _i : "0");
      const max = Number((_j = surface.dataset.instrumentElevationMax) != null ? _j : "90");
      const span = max - min;
      const radius = span > 0 ? clampUnit((max - elevation) / span) : 0;
      const bearing = azimuth * Math.PI / 180;
      surface.style.setProperty(
        "--instrument-light-x",
        String(settle(0.5 + 0.5 * radius * Math.cos(bearing)))
    ...
    const handleLightAxisFocus = (event) => {
      var _a2;
      const axis = (_a2 = event.target) == null ? void 0 : _a2.closest(
        AXIS_VALUE_SELECTOR
      );
      if (!axis || !root.contains(axis)) return;
      lightAxisFocusStart.set(axis, readLightAxis(axis).value);
    };
    const handleLightAxisKeyDown = (event) => {
      var _a2, _b2;
      const axis = (_a2 = event.target) == null ? void 0 : _a2.closest(
        AXIS_VALUE_SELECTOR
      );
      if (!axis || !root.contains(axis)) return;
      if (axis.closest("[data-instrument-disabled]")) return;
      const { max, min, step, value, wrap } = readLightAxis(axis);
      const multiplier = event.altKey ? 10 : event.shiftKey ? 0.1 : 1;
      const bound = (raw) => {
        if (!wrap) return settle(Math.min(max, Math.max(min, raw)));
        const span = max - min;
        if (!(span > 0)) return min;
        return settle(min + ((raw - min) % span + span) % span);
      };
      let next = null;
      switch (event.key) {
        case "Escape":
          next = (_b2 = lightAxisFocusStart.get(axis)) != null ? _b2 : value;
          break;
        case "Home":
          next = min;
          break;
        case "End":
          next = wrap ? bound(max - step) : max;
          break;
        case "ArrowRight":
        case "ArrowUp":
          next = bound(value + step * multiplier);
          break;
        case "ArrowLeft":
        case "ArrowDown":
          next = bound(value - step * multiplier);
    ...
    const handlePadPointerDown = (event) => {
      ...
      const surface = (_b2 = event.target) == null ? void 0 : _b2.closest(
        ".instrument-vector-pad-surface, .instrument-light-pad-surface, .instrument-angle-dial"
      );
    ...
    root.addEventListener("keydown", handleLightAxisKeyDown);
    root.addEventListener("focusin", handleLightAxisFocus);
    ...
    return {
      ...
      destroy() {
        ...
        root.removeEventListener("keydown", handleLightAxisKeyDown);
        root.removeEventListener("focusin", handleLightAxisFocus);
    ...
    root.innerHTML = `
		...
		<div class="instrument-shell" data-instrument-overview="visible" data-instrument-inspector="wide" data-instrument-active-sheet="none">
			...
			<main class="instrument-work" aria-label="Work">
				...
				<fieldset class="instrument-vector-pad" aria-label="Vector"><div class="instrument-vector-pad-surface" tabindex="-1"><div class="instrument-vector-pad-indicator" style="--instrument-pad-x:.4;--instrument-pad-y:.6"></div></div><div class="instrument-vector-pad-axes"><div class="instrument-vector-pad-axis"><span class="instrument-vector-pad-axis-label">X</span><div class="instrument-vector-pad-axis-value instrument-tabular" role="slider" tabindex="0" data-instrument-step="0.01" aria-label="X" aria-valuemin="0" aria-valuemax="1" aria-valuenow="0.4">0.40</div></div><div class="instrument-vector-pad-axis"><span class="instrument-vector-pad-axis-label">Y</span><div class="instrument-vector-pad-axis-value instrument-tabular" role="slider" tabindex="0" data-instrument-step="0.01" aria-label="Y" aria-valuemin="0" aria-valuemax="1" aria-valuenow="0.6">0.60</div></div></div></fieldset>
				<fieldset class="instrument-light-pad" aria-label="Key light"><div class="instrument-light-pad-surface" tabindex="-1" data-instrument-elevation-min="0" data-instrument-elevation-max="90" data-instrument-azimuth="135" style="--instrument-light-x:.32;--instrument-light-y:.32"><div class="instrument-light-pad-sphere" aria-hidden="true"></div><div class="instrument-light-pad-indicator"></div></div><output class="instrument-light-pad-reading">From the upper left, 45 degrees above the horizon</output><div class="instrument-light-pad-axes"><div class="instrument-light-pad-axis"><span class="instrument-light-pad-axis-label">Azimuth</span><div class="instrument-light-pad-axis-value instrument-tabular" role="slider" tabindex="0" data-instrument-step="1" data-instrument-wrap="true" aria-label="Azimuth" aria-valuemin="0" aria-valuemax="360" aria-valuenow="135" aria-valuetext="135 degrees">135<span class="instrument-light-pad-axis-unit" aria-hidden="true">°</span></div></div><div class="instrument-light-pad-axis"><span class="instrument-light-pad-axis-label">Elevation</span><div class="instrument-light-pad-axis-value instrument-tabular" role="slider" tabindex="0" data-instrument-step="1" aria-label="Elevation" aria-valuemin="0" aria-valuemax="90" aria-valuenow="45" aria-valuetext="45 degrees">45<span class="instrument-light-pad-axis-unit" aria-hidden="true">°</span></div></div></div></fieldset>
				<div class="instrument-angle" data-instrument-face="dial" data-instrument-wrap="true" style="--instrument-angle:118deg;--instrument-angle-size:72px"><div class="instrument-angle-dial"><div class="instrument-angle-dial-indicator"></div></div><div class="instrument-angle-row"><label class="instrument-angle-label" for="portable-angle">Sheen</label><output class="instrument-angle-value instrument-tabular" for="portable-angle">118<span class="instrument-angle-unit" aria-hidden="true">°</span></output><input class="instrument-range instrument-angle-input" id="portable-angle" type="range" min="0" max="360" step="1" value="118" data-instrument-step="1" data-instrument-wrap="true" aria-valuetext="118 degrees"></div></div>
  ...
  function setInstrumentVectorPad(pad, value) {
    var _a, _b;
    const axes = pad.querySelectorAll(
      ".instrument-vector-pad-axis-value"
    );
    const indicator = pad.querySelector(
      ".instrument-vector-pad-indicator"
    );
    const readings = [value.x, value.y];
    const units = [];
    axes.forEach((axis, index) => {
      var _a2, _b2;
      ...
      const decimals = String(step).includes(".") ? String(step).split(".")[1].length : 0;
      axis.setAttribute("aria-valuenow", String(reading));
      const unitLabel = axis.querySelector(".instrument-vector-pad-axis-unit");
      axis.textContent = reading.toFixed(decimals);
      if (unitLabel) axis.appendChild(unitLabel);
      units[index] = max === min ? 0 : Math.min(1, Math.max(0, (reading - min) / (max - min)));
    });
    indicator == null ? void 0 : indicator.style.setProperty("--instrument-pad-x", String((_a = units[0]) != null ? _a : 0.5));
    indicator == null ? void 0 : indicator.style.setProperty("--instrument-pad-y", String((_b = units[1]) != null ? _b : 0.5));
  }
  function angleRotation(value, origin, clockwise) {
    if (origin === "top") return clockwise ? value : -value;
    return clockwise ? value + 90 : 90 - value;
  }
  function setInstrumentAngle(control, value) {
    ...
    const readout = control.querySelector(".instrument-angle-value");
    if (readout) {
      const unit = readout.querySelector(".instrument-angle-unit");
      readout.textContent = value.toFixed(decimals);
      if (unit) readout.appendChild(unit);
    }
    const origin = control.dataset.instrumentOrigin === "right" ? "right" : "top";
    const clockwise = control.dataset.instrumentClockwise !== "false";
    control.style.setProperty(
      "--instrument-angle",
      `${angleRotation(value, origin, clockwise)}deg`
    );
  }
  function setInstrumentLightPad(pad, value) {
    var _a, _b;
    const surface = pad.querySelector(
      ".instrument-light-pad-surface"
    );
    const axes = pad.querySelectorAll(
      ".instrument-light-pad-axis-value"
    );
    const settleValue = (n) => Number(n.toFixed(10));
    const readings = [value.azimuth, value.elevation];
    axes.forEach((axis, index) => {
      var _a2;
      const reading2 = readings[index];
      if (reading2 === void 0) return;
      const step = Number((_a2 = axis.dataset.instrumentStep) != null ? _a2 : "1");
      const decimals = String(step).includes(".") ? String(step).split(".")[1].length : 0;
      axis.setAttribute("aria-valuenow", String(reading2));
      axis.setAttribute("aria-valuetext", `${reading2.toFixed(decimals)} degrees`);
      const unit = axis.querySelector(".instrument-light-pad-axis-unit");
      axis.textContent = reading2.toFixed(decimals);
      if (unit) axis.appendChild(unit);
    });
    const reading = pad.querySelector(".instrument-light-pad-reading");
    if (reading) reading.textContent = describeInstrumentLight(value);
    if (!surface) return;
    surface.dataset.instrumentAzimuth = String(value.azimuth);
    const min = Number((_a = surface.dataset.instrumentElevationMin) != null ? _a : "0");
    const max = Number((_b = surface.dataset.instrumentElevationMax) != null ? _b : "90");
    const span = max - min;
    const radius = span > 0 ? Math.min(1, Math.max(0, (max - value.elevation) / span)) : 0;
    if (span > 0 && min < 0 && max > 0) {
      surface.style.setProperty(
        "--instrument-light-horizon",
        `${(max / span * 50).toFixed(4)}%`
      );
    } else {
      surface.style.removeProperty("--instrument-light-horizon");
    }
    const bearing = value.azimuth * Math.PI / 180;
    surface.style.setProperty(
      "--instrument-light-x",
      String(settleValue(0.5 + 0.5 * radius * Math.cos(bearing)))
    );
    surface.style.setProperty(
      "--instrument-light-y",
      String(settleValue(0.5 - 0.5 * radius * Math.sin(bearing)))
    );
  }
  function describeInstrumentLight(value) {
    const quarters = [
      "the right",
      "the upper right",
      "above",
    ...
    const bearing = (value.azimuth % 360 + 360) % 360;
    const quarter = quarters[Math.round(bearing / 45) % 8];
    const rounded = Math.round(value.elevation);
    const height = rounded === 0 ? "level with the horizon" : rounded > 0 ? `${rounded} degrees above the horizon` : `${Math.abs(rounded)} degrees below the horizon`;
    return `From ${quarter}, ${height}`;
  }
  const SUPERMEGA_INSTRUMENT2 = Object.freeze({
    ...
    setLightPad: setInstrumentLightPad,
    setAngle: setInstrumentAngle,
    setVectorPad: setInstrumentVectorPad,
    angleRotation,
    describeLight: describeInstrumentLight,
