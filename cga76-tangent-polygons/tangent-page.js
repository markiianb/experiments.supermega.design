(function tangentPolygonPage(global) {
	...
	toggle.addEventListener("click", () => adapter.execute(panel.classList.contains("hidden") ? "show-controls" : "hide-controls")); $("load").addEventListener("click", () => loadSerialized($("config").value, "CONFIG")); $("select-text").addEventListener("click", () => { $("config").focus(); $("config").select(); });
